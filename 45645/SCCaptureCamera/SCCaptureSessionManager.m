...
- (void)switchGrid:(BOOL)toShow {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i == 0 || i == 1) {//画横线
            frame = CGRectMake(0, headHeight + (i + 1) * eachAreaLength + 20, squareLength, 1);
        } else {
            frame = CGRectMake((i + 1 - 2) * eachAreaLength, headHeight + 30, 1, squareLength + 20);
        }
        [SCCommon drawALineWithFrame:frame andColor:[UIColor redColor] inLayer:_preview.layer];
    }
}
